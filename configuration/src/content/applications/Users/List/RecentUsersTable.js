  ...
  return tableData.filter((tbData)=>{
    ...
    if (
      filters.searchTerm &&
        ...
        (!`${tbData.employeeNumber}`.toLowerCase().includes(search)) &&
        (!`${tbData.area_desc}`.toLowerCase().includes(search)) &&
...
const RecentUsersTable = ({ tableData, setRefreshUsers }) => {
  ...
  const handleClickSwitchEdit = (userId) => {
    if(userId === 1) return;
    navigate(`/management/users/list/${userId}`, { replace: false});
  }
  const handleDisableUsers = async (userIds) => {
    const _userData = {
      idList: userIds
    }
    try {
  ...
  return (
    <Card>
      {selectedBulkActions && (
        <Box flex={1} p={2}>
          <BulkActions selected={selectedTableData} handleDisableUsers={handleDisableUsers} />
        </Box>
      )}
      {!selectedBulkActions && (
        <CardHeader
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right">Auth type</TableCell>
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right">
                    {tbData.isLDAPAuth ? <Label color="success">LDAP</Label> : <Label color="warning">Credentials</Label>}
                  </TableCell>
                  <TableCell align="right">
                  ...
                  <TableCell align="right">
                    ...
                    <Tooltip title="Delete User" arrow>
                      <IconButton
                        ...
                        color="inherit"
                        size="small"
                        onClick={ev=>{handleDisableUsers([tbData.id])}}
                      >
