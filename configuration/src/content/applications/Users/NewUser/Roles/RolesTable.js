...
const applyFiltersBySearchTerm = (tableData, filters) => {
  return tableData.filter((tbData)=>{
    ...
    if (
      filters.searchTerm &&
        ((!`${tbData.name} ${tbData.lastName}`.toLowerCase().includes(search)) &&
        (!`${tbData.numEmp}`.toLowerCase().includes(search)) &&
        (!`${tbData.areaDesc}`.toLowerCase().includes(search)) &&
        (!(tbData.isLDAPAuth ? 'ldap' : 'credentials').includes(search)) &&
        (!(tbData.isEnabled ? 'active' : 'inactive').includes(search)))
      ){
        matches = false;
      }
    return matches;
...
const RolesTable = ({ tableData, GetUser, userId, close }) => {
  ...
  const handleBulkAddUserRoles = async (roleIds) => {
    const moduleIds = tableData.filter(tb => roleIds.includes(tb.roleId)).map(ur => ur.moduleId);
    console.log("moduleIDs:"+moduleIds)
    const tempGroupModule = _.groupBy(moduleIds, this);
    const arrSizeGroup = Object.keys(tempGroupModule).map(k=> tempGroupModule[k].length);

    if(arrSizeGroup.find(e => e > 1) != null){
      notifyError("Only one role can be added for each module");
      return;
    }

    const _userRolesData = {
      userRoles: roleIds.map(v=>({
        roleId: v,
        userId: userId,
        createdBy: 1
      }))
    ...
    try {
      let res = await myAxios('configuration', auth()).post(`Users/bulkUserRole`, _userRolesData);
  ...
  return (
    <Card>
      {selectedBulkActions && (
        <Box flex={1} p={2}>
          <BulkActions selected={selectedTableData} handleBulkAddUserRoles={handleBulkAddUserRoles} />
        </Box>
      )}
      {!selectedBulkActions && (
        <CardHeader
                ...
                >
                  ...
                  <TableCell>
                    <Typography
                      ...
                      noWrap
                    >
                      {tbData.roleViews.map(rv=>`[${rv.path}]`).join(', ')}
                    </Typography>
                  </TableCell>
                </TableRow>
              );
