...
const RecentViewsTable = ({ tableData, setRefreshViews }) => {
  ...
  const handleClickSwitchEdit = (viewId) => {
    navigate(`/management/views/list/${viewId}`, { replace: false});
  }
  const handleDisableViews = async (viewIds) => {
    const _viewData = {
      idList: viewIds
    }
    try {
  ...
  return (
    <Card>
      {selectedBulkActions && (
        <Box flex={1} p={2}>
          <BulkActions selected={selectedTableData} handleDisableViews={handleDisableViews} />
        </Box>
      )}
      {!selectedBulkActions && (
        <CardHeader
          ...
          title={
            <Box component="div" sx={{width:300, display: 'flex', alignItems: 'center'}}>
              ...
              <BootstrapInput sx={{width:"100%"}} placeholder="Search by view"
                  ...
                  <TableCell align="right">
                    ...
                    <Tooltip title="Delete View" arrow>
                      <IconButton
                        ...
                        color="inherit"
                        size="small"
                        onClick={ev=>{handleDisableViews([tbData.id])}}
                      >
...
RecentViewsTable.propTypes = {
  tableData: PropTypes.array.isRequired
};

RecentViewsTable.defaultProps = {
  tableData: []
};
