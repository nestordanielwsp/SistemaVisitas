...
const RecentModulesTable = ({ tableData, setRefreshModules }) => {
  ...
  const handleClickSwitchEdit = (moduleId) => {
    navigate(`/management/modules/list/${moduleId}`, { replace: false});
  }
  const handleDisableModules = async (moduleIds) => {
    const _moduleData = {
      idList: moduleIds
    }
    try {
  ...
  return (
    <Card>
      {selectedBulkActions && (
        <Box flex={1} p={2}>
          <BulkActions selected={selectedTableData} handleDisableModules={handleDisableModules} />
        </Box>
      )}
      {!selectedBulkActions && (
        <CardHeader
          ...
          title={
            <Box component="div" sx={{width:300, display: 'flex', alignItems: 'center'}}>
              ...
              <BootstrapInput sx={{width:"100%"}} placeholder="Search by module"
            ...
            {paginatedTableData.map((tbData) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right">
                    <Tooltip title="Edit Modulo" arrow>
                      <IconButton
                        sx={{
                          '&:hover': {
                    ...
                    <Tooltip title="Delete Modulo" arrow>
                      <IconButton
                        sx={{
                        ...
                        color="inherit"
                        size="small"
                        onClick={ev=>{handleDisableModules([tbData.id])}}
                      >
...
RecentModulesTable.propTypes = {
  tableData: PropTypes.array.isRequired
};

RecentModulesTable.defaultProps = {
  tableData: []
};

export default RecentModulesTable;
