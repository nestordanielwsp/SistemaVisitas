...
const RecentRolesTable = ({ tableData, setRefreshRoles }) => {
  ...
  const handleClickSwitchEdit = (roleId) => {
    navigate(`/management/roles/list/${roleId}`, { replace: false});
  }
  const handleDisableRoles = async (roleIds) => {
    const _roleData = {
      idList: roleIds
    }
    try {
      let res = await myAxios('configuration').put(`Roles/bulkDisable`, _roleData);
  ...
  return (
    <Card>
      {selectedBulkActions && (
        <Box flex={1} p={2}>
          <BulkActions selected={selectedTableData} handleDisableRoles={handleDisableRoles} />
        </Box>
      )}
      {!selectedBulkActions && (
        <CardHeader
          ...
          title={
            <Box component="div" sx={{width:300, display: 'flex', alignItems: 'center'}}>
              ...
              <BootstrapInput sx={{width:"100%"}} placeholder="Search by role"
                  ...
                  <TableCell align="right">
                    ...
                    <Tooltip title="Delete Role" arrow>
                      <IconButton
                        ...
                        color="inherit"
                        size="small"
                        onClick={ev=>{handleDisableRoles([tbData.id])}}
                      >
